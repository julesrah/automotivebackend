...
router.get(`/`, async (req, res) => {
    const borrowList = await Borrow.find().populate('user', 'name').sort({ 'dateBorrowed': -1 });
...
router.get(`/:id`, async (req, res) => {
    const borrow = await Borrow.findById(req.params.id)
        .populate('user', 'name')
        .populate({
            path: 'borrowItems', populate: {
                path: 'tool', populate: 'category'
            }
        });
...
router.post('/', async (req, res) => {
    const borrowItemsIds = Promise.all(req.body.borrowItems.map(async (borrowItem) => {
        ...
        if (!borrow)
            return res.status(400).send('The borrow cannot be created!')
    ...
    router.put('/:id', async (req, res) => {
        ...
        if (!borrow)
            return res.status(400).send('the Borrow cannot be update!')
    
        res.send(borrow);
    })

    router.delete('/:id', (req, res) => {
        Borrow.findByIdAndRemove(req.params.id).then(async borrow => {
            if (borrow) {
                await borrow.borrowItems.map(async borrowItem => {
                    await BorrowItem.findByIdAndRemove(borrowItem)
                })
                return res.status(200).json({ success: true, message: 'the borrow is deleted!' })
            } else {
    ...
    router.get(`/get/userborrows/:userid`, async (req, res) => {
        const userBorrowList = await Borrow.find({ user: req.params.userid }).populate({
            path: 'borrowItems', populate: {
                path: 'tool', populate: 'category'
            }
        }).sort({ 'dateBorrowed': -1 });
